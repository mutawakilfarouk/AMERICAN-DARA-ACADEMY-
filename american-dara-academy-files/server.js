    ...
    if (pathname === '/api/students' && req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          if (!student.id) {
            student.id = `ADA-${student.division === 'Lower School' ? 'LS' : (student.division === 'Middle School' ? 'MS' : 'HS')}-${String(db.students.length + 1).padStart(3, '0')}`;
          }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`\n======================================================`);
  console.log(`🦅 American Dara Academy - Server Online on All Interfaces!`);
